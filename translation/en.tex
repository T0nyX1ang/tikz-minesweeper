...
\def\TransMacroValidateInRangeContent{Judge whether \meta{\#1} is between the lower bound \meta{\#2} and the upper bound \meta{\#3} inclusively. If \meta{\#2} is empty, the function will only judge whether \meta{\#1} is less than \meta{\#3}; if \meta{\#3} is empty, the function will only judge whether \meta{\#1} is greater than \meta{\#2}.}
...
\def\TransMacroRowContent{Draw the row of cells, accepting two parameters \meta{\#1}, \meta{\#2}. \meta{\#1} represents the row number or column number, \meta{\#2} represents the content of the cells.}
